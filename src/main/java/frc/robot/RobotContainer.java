  ...
  public RobotContainer() {
    ...
    intake.setDefaultCommand(
     ...
     pShooter.setDefaultCommand(
       new Shooterarm( pShooter ,shooterarm ));
...
// works
    ...
    // new JoystickButton(operator, XboxContro  ller.Axis.kRightY.value)
    //   .whileTrue(new Shooterarm(pShooter , shooterarm));
    

// intake
    // if (toggleManual.getAsBoolean() == true){ 
    //   if (Toggle == true) {
    //     Toggle = false; 
    //   }
    //   else { 
  ...
  //  new Trigger(() -> (Math.abs(operator.getRawAxis(XboxController.Axis.kLeftY.value)) > 0))
